...
def get_or_create_thread(ad_id, user1_id, user2_id):
    ...
    sql = (
        ...
        "WHERE ad_id = ? AND ((user1_id = ? AND user2_id = ?) "
...
def get_user_threads(user_id):
    ...
    sql = (
        "SELECT t.id, t.ad_id, m.title AS ad_title, u.username AS partner "
        "FROM threads t "
...
def get_threads_by_message(ad_id):
    ...
    sql = (
        "SELECT t.id, t.user1_id, t.user2_id, u1.username AS user1_name, u2.username AS user2_name "
        "FROM threads t "
...
def get_unread_messages(user_id):
    ...
    sql = (
        ...
        "JOIN users u ON tm.sender_id = u.id "
        "WHERE (t.user1_id = ? OR t.user2_id = ?) "
        "AND tm.sender_id != ? AND tm.read = 0 "
...
def mark_thread_as_read(thread_id, user_id):
    ...
    sql = "UPDATE thread_messages SET read = 1 WHERE thread_id = ? AND sender_id != ?"
